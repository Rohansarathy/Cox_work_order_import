...
def validate_excel(driver, mode, COX_FILE, target_date, credentials,  download_folder, log_file, previous_date, current_hour):
    print("Starting Excel validation process...")
    log_message(log_file, "Starting Excel validation process...")
    try:
        if not os.path.exists(download_folder):
            raise FileNotFoundError(f"Download folder not found: {download_folder}")

        if mode not in ["current", "previous"]:
            raise ValueError("Mode must be either 'current' or 'previous'")
        ...
        def extract_tech_id(value):
            ...
            if match:
                log_message(log_file, f"Dash format matched: {value}")
                return match.group(1)
            
            match = re.search(r'(\d+)\s*\(', value)
            if match:
                log_message(log_file, f"Parenthesis format matched: {value}")
                return match.group(1)
            
            log_message(log_file, f"\033[91mNo match for Tech IDs: {value}\033[0m")
            return None
        ...
        for file in csv_files:
            ...
            try:
                ...
                raise Exception(f"Failed reading CSV file: {file}. Error: {str(e)}")
        ...
        log_message(log_file, f"Total rows merged: {len(merged_df)}")
        if merged_df.empty:
        ...
        filenames = [COX_FILE]
        if "HST/UHT" not in df.columns:
            raise KeyError("'HST/UHT' column missing")
        ...
        for filename in filenames:
            ...
            if mask.any():
                ...
                log_message(log_file, f"Found '{target_word}' in {filename}")
                print(df[mask])
            ...
            if mode == "previous":
                ...
                log_message(log_file, f"Processing previous day's file with date: {process_date}")
            else:
                process_date = today.strftime("%m/%d/%Y")
                print(f"Processing current day's file with date: {process_date}")
                log_message(log_file, f"Processing current day's file with date: {process_date}")
            df["Date"] = process_date

            if "Service window" not in df.columns:
                raise KeyError("'Service window' column missing")
            ...
            if "Service window" in df.columns:
                ...
                log_message(log_file, f"Total rows before: {len(df)}")
                df = df[
                    df["Service window"].notna() &
                    (df["Service window"].astype(str).str.strip() != "")
                ]
                print("Total rows after:", len(df))
                log_message(log_file, f"Total rows after: {len(df)}")
                df.to_excel(filename, index=False)
            else:
                print("'Service window' column not found in the DataFrame.")
                log_message(log_file, "'Service window' column not found in the DataFrame.")
            ...
            log_message(log_file, f"Finished processing: {filename}")
            print("---------------------------------------------------")
            log_message(log_file, "---------------------------------------------------")
    except Exception as e:
        print(f"VALIDATION FAILED: {str(e)}")
        log_message(log_file, f"VALIDATION FAILED: {str(e)}")
        raise 

    cox_bulk_import(driver, mode, COX_FILE, target_date, credentials, log_file, previous_date, current_hour)